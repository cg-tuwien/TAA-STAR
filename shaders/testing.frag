...
{
	mat4 vmMatrix = uboMatUsr.mViewMatrix * gModelMatrix;
...
void main()
{
	gMaterialIndex = materialIndex[fs_in.drawID];
	gModelMatrix = attrib[attrib_base[fs_in.drawID] + fs_in.instanceIndex].modelMatrix;
	...
	oFragVelocity = vec4(motionVector, 0);
	...
	if (uboMatUsr.mUserInput.z < 1.f) {
		...
		vec3 normalWS = normalize(mat3(inverse(transpose(gModelMatrix))) * normalize(fs_in.normalOS));
		oFragColor = vec4(normalWS.xyz, 1.0);
